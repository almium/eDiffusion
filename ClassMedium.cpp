...
//	cout	<< "(Nx, Ny, Nz)=(" << Nx << ", "<< Ny << ", "<< Nz<< ")" << endl;
	...
	if (ClassQuencher::ASradius>0)
	{	
		ASgrid 	= new Bool3D(Nx,Ny,Nz);           /* allocate memory for Action Sphere medium. */
	}else{
		ASgrid = grid;
	}
...
//	qn = 5000; // debug
	...
	counter = PixelSphere (qR,gi,gj,gk);
	cout << "quencher includes " << counter << " grid cells \n";

	if (ClassQuencher::ASradius>0){	
		qR =ClassQuencher::ASradius + ClassExciton::radius; 
		counter = PixelSphere (qR,ASgi,ASgj,ASgk);
		cout << "Action sphere includes " << counter << " grid cells \n";
	}
...
//	ReadMediumBMP("medium.P3HT-ZnO");	
	...
	{	
		...
		Save2DgridBMP(grid,Nz/2,fname);
	}

	if (ClassQuencher::ASradius>0){	
		char fname[128];
		sprintf(fname,  "Grid Action Sphere.Volume frac %2.5f.bmp",frac);
		Save2DgridBMP(ASgrid,Nz/2,fname);
	}
...
void
ClassMedium::PlaceInterfaceQuenchers ( double iConc_cm2 ) /* this function allocates memory for Bool3D*/
{
...
//	cout	<< "(Nx, Ny, Nz)=(" << Nx << ", "<< Ny << ", "<< Nz<< ")" << endl;
	...
	qn = (int) X * Y * 1e-14 * iConc_cm2;


//	qn = 5000; // debug
	cout	<< "number of interface quencers at z=0: " << qn  << endl;
	...
	/* Save 2D BMP image of the grid nz=0 and nz = Z */

	char fname[128];


	sprintf(fname,  "Grid Image.z=0.iConc=%.3e.bmp",iConc_cm2);
	Save2DgridBMP(grid,0,fname);
	/*
	sprintf(fname,  "Grid Image.z=Z.iConc %.3e.bmp",iConc_cm2);
	Save2DgridBMP(grid,Nz-1,fname);
*/
 ...
 *--------------------------------------------------------------------------------------
 *       Class:  ClassMedium
 *      Method:  ClassMedium :: PixelSphere
 * Description:  Sets the list of coordinates of a pixelated sphre of a given radius qR 
 *               to vector vi, vj, vk. Returns number of grid cells.
 *--------------------------------------------------------------------------------------
 */
	int	
ClassMedium::PixelSphere ( double qR, vector<int>&  vi, vector<int>& vj, vector<int>& vk )
{
	int i,j,k;
	int qRadiusN;                            /* quencher radius expressed in grid points */
	double qR2;                         /*  quencher radius squared */
	int counter=0;	
...
//				   )
				...
				{
					vi.push_back(i); /* memorize coordinates i,j,k */
					vj.push_back(j);
					vk.push_back(k);
					counter++;
					//cout << 'x';
	...
	return counter;
}		/* -----  end of method ClassMedium::PixelSphere  ----- */
 ...
 *--------------------------------------------------------------------------------------
 *       Class:  ClassMedium
 *      Method:  ClassMedium :: PlaceInterfaceQuenchers
 * Description:  Places the quenchers at z=0 on a square grid
 *--------------------------------------------------------------------------------------
 */

bool
ClassMedium::PlaceInterfaceQ ()
{
//	unsigned i;
	int i, a, b, c;                                /* iterators */
	double d;
	int qx, qy;

	if (qn<1) return false;

	d = sqrt (1.0 * X * Y / (1.0*qn));
	...
	cout << "placing " << qn << " quenchers on a "<< qx << "x" << qy << "grid. total grid points: " << qx*qy << "\n";

	int skipN = qx * qy - qn;
	vector <int> skipList;

	// generate the list of randomly skipped items
	for (i=0; i < skipN; i++){
		int randI;
		do {
			randI = Rnd.iRnd(qn);
		} while (find(skipList.begin(), skipList.end(),randI) != skipList.end());

		skipList.push_back(randI);
		//cout << "skipping quencher " << randI << "\n";
	...
	for (a =0; a < qx; a++){
		for (b=0; b < qy; b++){
			int num = b*qx + a;
			// check for the skip list
			if (find(skipList.begin(), skipList.end(),num) == skipList.end()){
				q[cnt].SetCoordinates(a*d, b*d, 0.0, X, Y, Z);
				SetOnGrid(&q[cnt]);
				cnt++;
			}
		}
	...
	cout << "Placed " << cnt << " quenchers \n";

	// here we must clear the space near z=Z

	double qR = ClassExciton::radius + ClassQuencher::radius;
	int qRadiusN =qR/dX + 2; // +2 to be sure

	for (c = Nz - qRadiusN; c < Nz; c++ ){
		for (b = 0; b < Ny; b++){
			for (a = 0; a < Nx; a++){
				if (grid->get(a,b,c) == true){
					grid->set(a,b,c, false);
					numQ = numQ - 1;
				}
			}
		}
...
//	Action sphere
	if (ClassQuencher::ASradius>0){ // this saves a lot of time when action sphere is not needed	
		for ( i = 0; i < (int) ASgi.size() ; i += 1 )
       		{
			ASgrid->set(nx+ASgi[i], ny+ASgj[i], nz+ASgk[i], true); 
								/* Class Bool3D takes care 
			       					about periodic boundary conditions */
		}
...
bool
ClassMedium::GridQuenching (Bool3D* grid1, ClassExciton* e)
{
	int nx, ny, nz;
...
void
ClassMedium::Save2DgridBMP ( Bool3D* grid1, int zindex, char* filename )
{
	int i,j;
	string fname="";
	time_t rawtime;
